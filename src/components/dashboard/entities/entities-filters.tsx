...
import { useEntitiesSelection } from './entities-selection-context';
...
export interface EntitiesFiltersProps {
  filters?: Filters;
  sortDir?: SortDir;
}

export function EntitiesFilters({ filters = {}, sortDir = 'desc' }: EntitiesFiltersProps): React.JSX.Element {
  const { email, phone, status } = filters;

  const router = useRouter();

  const selection = useEntitiesSelection();
  ...
  const updateSearchParams = React.useCallback(
    (newFilters: Filters, newSortDir: SortDir): void => {
      ...
      router.push(`${paths.dashboard.entities.list}?${searchParams.toString()}`);
    },
    [router]
  );
