...
export function ReportCreateForm(): React.JSX.Element {
  ...
  const onSubmit = React.useCallback(
    async (_: Values): Promise<void> => {
      try {
        ...
        router.push(paths.dashboard.reports.details('1'));
      } catch (err) {
        logger.error(err);
    ...
    <form onSubmit={handleSubmit(onSubmit)}>
      <Card>
        ...
        <CardActions sx={{ justifyContent: 'flex-end' }}>
          <Button color="secondary" component={RouterLink} href={paths.dashboard.reports.list}>
            Cancel
          </Button>
